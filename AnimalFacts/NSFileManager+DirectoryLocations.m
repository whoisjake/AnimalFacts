...
// documentsDirectory
//
// Returns the path to the documentSupportDirectory (creating it if it doesn't
// exist).
//
- (NSString *)documentsDirectory
{
    NSString *applicationSupportPath = [self applicationSupportDirectory];
    NSString *result = [applicationSupportPath stringByAppendingPathComponent:@"Documents"];

	NSError *error = nil;
	BOOL success = [self
                    createDirectoryAtPath:result
                    withIntermediateDirectories:YES
                    attributes:nil
                    error:&error];
	if (!success)
